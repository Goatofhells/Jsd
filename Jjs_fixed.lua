...
if #v59 ~= 0 then
    ...
    v83.RenderStepped:Connect(function()
        ...
        if lockOnEnabled and vu87 then
            if lockOnMethod == "Camera" then
                if vu87.Character and vu87.Character:FindFirstChild("HumanoidRootPart") then
    ...
    local function HookQTE()
        for _, obj in pairs(getgc(true)) do
            if type(obj) == "table" then
                pcall(function()
                    if rawget(obj, "InRange") and type(obj.InRange) == "function" then
                        if not rawget(obj, "_OriginalInRange") then
                            obj._OriginalInRange = obj.InRange
                        end
                        obj.InRange = function(...)
                            if qteEnabled then
                                return true
                            end
                            return obj._OriginalInRange(...)
                        end
                    end
                    if rawget(obj, "Bind") and type(obj.Bind) == "function" then
                        if not rawget(obj, "_OriginalBind") then
                            obj._OriginalBind = obj.Bind
                        end
                        obj.Bind = function(self2)
                            obj._OriginalBind(self2)
                            if qteEnabled and self2.OnPressed then
                                self2.OnPressed:Fire()
                            end
                        end
                    end
                end)
    ...
    local function RestoreQTE()
        for _, obj in pairs(getgc(true)) do
            if type(obj) == "table" then
                pcall(function()
                    if rawget(obj, "_OriginalInRange") then
                        obj.InRange = obj._OriginalInRange
                    end
                    if rawget(obj, "_OriginalBind") then
                        obj.Bind = obj._OriginalBind
                    end
                end)
            end
        end
    ...
    local vu351 = require(game:GetService("ReplicatedStorage").Modules.Player.Character.Combat)
    ...
    v395:Toggle({
        Title = "Auto QTE",
        Desc = "automatically passes all QTE/timing minigames instantly",
        Default = false,
        Callback = function(p)
            qteEnabled = p
            if p then
    ...
    local antiStunHumanoid = nil
    local antiStunOriginalSpeed = 16
    local antiStunConnection = nil

    local function updateAntiStunHumanoid()
        local char = game:GetService("Players").LocalPlayer.Character
        if char then
            antiStunHumanoid = char:FindFirstChildOfClass("Humanoid")
            if antiStunHumanoid then
                antiStunOriginalSpeed = antiStunHumanoid.WalkSpeed > 0 and antiStunHumanoid.WalkSpeed or antiStunOriginalSpeed
            end
        end
    end

    updateAntiStunHumanoid()
    game:GetService("Players").LocalPlayer.CharacterAdded:Connect(function(c)
        c:WaitForChild("Humanoid", 5)
        updateAntiStunHumanoid()
    end)

    antiStunConnection = v48.Heartbeat:Connect(function()
        if antiStunActive and antiStunHumanoid then
            if antiStunHumanoid.WalkSpeed == 0 then
                antiStunHumanoid.WalkSpeed = antiStunOriginalSpeed
            else
                antiStunOriginalSpeed = antiStunHumanoid.WalkSpeed
            end
        end
    end)
    ...
    v395:Dropdown({
        Title = "Method",
        Desc = "Character = rotates your HRP | Camera = rotates your camera view",
